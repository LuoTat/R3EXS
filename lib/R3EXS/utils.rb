...
=end
        ...
        # @raise [TypeError] 如果 object 的类型不正确
        # @return [Void]
        def Utils.check_type(object, file_basename, is_compact, module_name)
            ...
            when :R3EXS
                ...
                raise ArgumentError, "Invalid module name: #{module_name}"
            end

            if object.is_a?(Array)
                if is_compact
                    object.compact.all? { |item| item.is_a?(matched_class) } or raise TypeError, "Invalid JSON file: #{file_basename}, it's not Array<#{matched_class}>"
                else
                    object.all? { |item| item.is_a?(matched_class) } or raise TypeError, "Invalid JSON file: #{file_basename}, it's not Array<#{matched_class}>"
                end
            elsif object.is_a?(Hash)
                if is_compact
                    object.compact.values.all? { |item| item.is_a?(matched_class) } or raise TypeError, "Invalid JSON file: #{file_basename}, it's not Hash<#{matched_class}>"
                else
                    object.values.all? { |item| item.is_a?(matched_class) } or raise TypeError, "Invalid JSON file: #{file_basename}, it's not Hash<#{matched_class}>"
                end
            else
                object.is_a?(matched_class) or raise TypeError, "Invalid JSON file: #{file_basename}, it's not #{matched_class}"
            end
        end
        ...
        def Utils.rpg_r3exs(object, file_basename, with_notes)
            ...
            if object.is_a?(Array)
                ...
                # 只有 RPG::Map 和 RPG::System 是单独一个对象，且不可能为 nil，因为 check_type 中对单独一个对象的检查会对 nil 抛出异常
        ...
        def Utils.all_rvdata2_files(target_dir)
            # 检查 target_dir 是否存在
            Dir.exist?(target_dir) or raise Errno::ENOENT, "Rvdata2 directory not found: #{target_dir}"
            ...
            Dir.each_child(target_dir) do |file_name|
                ...
                file_dir = File.join(target_dir, file_name)
                print "#{ESCAPE}#{BLUE_COLOR}Reading and Deserializing from #{RESET_COLOR}#{file_dir}...\r" if $global_options[:verbose]
                object = File.open(file_dir, 'rb') { |file| Marshal.load(file) }
                ...
                unless file_basename == 'Scripts'
                    ...
                    begin
                        ...
                        $stderr.puts "#{RED_COLOR}#{e.message}#{RESET_COLOR}"
                        $stderr.puts "The file #{GREEN_COLOR}#{file_basename}.rvdata2#{RESET_COLOR} is not a valid rvdata2 file"
                        next
                    end
                end
        ...
        def Utils.all_json_files(target_dir, module_name)
            # 检查 target_dir 是否存在
            Dir.exist?(target_dir) or raise Errno::ENOENT, "JSON directory not found: #{target_dir}"
            # 获取 target_dir 下的所有文件
            Dir.each_child(target_dir) do |file_name|
                ...
                file_dir = File.join(target_dir, file_name)
                print "#{ESCAPE}#{BLUE_COLOR}Reading and Deserializing #{RESET_COLOR}#{file_dir}...\r" if $global_options[:verbose]
                object = Oj.load_file(file_dir)
